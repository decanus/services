                ...
                match trade {
                    ...
                    Trade::Jit(jit) => {
                        ...
                        );
                        {
                            ...
                            jit.quoted_order.order = jit.quoted_order.order.receiver(Some(
                                config
                                    .blockchain
                            ...
                            let order = json!({
                                ...
                                "signature": if config.quote { "0x".to_string() } else { format!("0x{}", hex::encode(jit.quoted_order.order_signature_by_surplus_capturing_jit_order_owner(config.blockchain))) },
                                "signingScheme": if config.quote { "eip1271" } else { "eip712" },
                            });
